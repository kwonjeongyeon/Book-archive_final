...
public class MemberControllerImpl extends BaseController implements MemberController {
	...
	public ModelAndView login(@RequestParam Map<String, String> loginMap, HttpServletRequest request,
		...
		if (memberVO != null && memberVO.getMember_id() != null) {
			...
			// 조회한 회원 정보를 가져와 isLogOn 속성을 true로 설정하고 memberInfo 속성으로 회원 정보를 저장
			HttpSession session = request.getSession();
			session = request.getSession();
			session.setAttribute("isLogOn", true);
			session.setAttribute("memberInfo", memberVO);

			// 상품 주문 과정에서 로그인했으면 로그인 후 다시 주문 화면으로 진행하고 그 외에는 메인 페이지 표시
	...
	public ResponseEntity addMember(@ModelAttribute("memberVO") MemberVO _memberVO, HttpServletRequest request,
			HttpServletResponse response) throws Exception { // 회원가입 창에 전송된 정보를 _memberVO에 설정
	...
	public ResponseEntity overlapped(@RequestParam("id") String id, HttpServletRequest request,
		...
		String result = memberService.overlapped(id); // 아이디 중복검사
