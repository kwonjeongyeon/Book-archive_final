...
function fn_modify_order_state(order_id){
    ...
    //함수로 전달받은 셀렉트 박스에서 선택한 옵션의 인덱스로 선택한 배송 상태 값을 가져온다
	$.ajax({
		type : "post",
		async : false, //false인 경우 동기식으로 처리한다.
		url : "${contextPath}/admin/order/modifyDeliveryState.do",
		data : { //주문 번호와 배송 상태 값을 컨트롤러로 전송
			order_id:order_id,
				...
				<tbody>
					<tr class="dot_line">
						...
						<td width=10%><input type="button" value="배송수정"
							onClick="fn_modify_order_state('${deliveryInfo.order_id}')" /> <%-- 배송수정을 클릭하면 셀렉트 박스의 id 전달 --%>
						</td>
					</tr>
